...
const writeUrl = (originalUrl, shortUrl) => {
	return new Promise((resolve, reject) => {
		...
		client.hset(APP_URL_KEY, innerKey, originalUrl, (err, data) => {
			if (err) reject(err);
			resolve(data);
		});
	});
...
const initCount = shortUrl => {
	return new Promise((resolve, reject) => {
		let innerKey = shortUrl;
		client.hset(APP_CLICK_KEY, innerKey, 0, (err, data) => {
			if (err) reject(err);
			resolve(data);
		});
	});
...
const incrementCount = shortUrl => {
	return new Promise((resolve, reject) => {
		let innerKey = shortUrl;
		client.hincrby(APP_CLICK_KEY, innerKey, 1, (err, data) => {
			if (err) reject(err);
			resolve(data);
		});
	});
